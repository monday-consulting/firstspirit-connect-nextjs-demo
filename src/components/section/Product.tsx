...
const Product = ({ data }: ProductProps) => {
    ...
    const { richTextTypes, richTextContents, richTextData } =
        currentDataset.description.reduce(
            (acc, item) => {
    ...
    const favoriteButtonId = currentDataset.categories
        .map((item) => item.id)
        .join(" ");
    ...
    return (
        <section
            ...
            data-preview-id={currentDataset?.categories.map(
                (item) => item.previewId
            )}
        >
                        ...
                        <div className="py-6 lg:pr-32">
                            <div className="mb-4">
                                {categoryNames.map((category: string) => (
                                    <span
                                        key={category}
                                        className="mr-1 rounded-xl bg-gray-900 px-3 py-1 font-semibold text-white text-xs"
                                    >
                                        {category}
                                    </span>
                                ))}
                                <h1 className="mt-3 font-bold font-heading text-4xl">
                                    {currentDataset?.name}
                                </h1>
                            </div>
                            ...
                            <div className="mt-10 flex items-start border-gray-200 border-t py-4 pt-10">
