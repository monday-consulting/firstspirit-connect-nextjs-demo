...
export default function SimpleChatUI() {
    ...
    const systemPromptPresets = {
        ...
        ultra_aggressive:
            "Ultra Aggressive: Use EVERY MCP feature for EVERY response 🚨",
        aggressive:
            "Aggressive: Use ALL available MCP features for every response 🔥",
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (!res.ok) {
                    throw new Error(
                        `HTTP ${res.status} ${res.statusText} – Body: ${raw?.slice(0, 400)}`
                    );
                }
                let data: any = null;
                try {
                    ...
                    throw new Error(
                        `JSON parse failed. Body: ${raw?.slice(0, 400)}`
                    );
                }
        ...
        try {
            const response = await fetch("/api/mcp/chat", {
                ...
                body: JSON.stringify({
                    ...
                    useResources: selectedResources.length
                        ? selectedResources
                        : undefined,
                    usePrompts: selectedPrompts.length
                        ? selectedPrompts
                        : undefined,
                    customSystemPrompt:
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="mt-2 rounded border border-green-200 bg-green-50 p-2">
                                ...
                                <div className="space-y-1 text-green-700 text-xs">
                                    ...
                                    <div>
                                        ✅ Auto-loading ALL{" "}
                                        {availableResources.length} resources
                                    </div>
                                    <div>
                                        ✅ Auto-applying relevant prompts from{" "}
                                        {availablePrompts.length} available
                                    </div>
                                    <div>
                                        ✅ All {availableTools.length} tools
                                        ready for aggressive use
                                    </div>
                                    <div>
                                        ✅ System prompt:{" "}
                                        <strong>
                                            {systemPromptPresets[
                                                selectedPreset as keyof typeof systemPromptPresets
                                            ] || "Custom"}
                                        </strong>
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <span className="ml-1 text-blue-600">
                                {connectedServers.length > 0
                                    ? `${connectedServers.length} server(s)`
                                    : "None"}
                            </span>
                        </div>
                        <div>
                        ...
                        {messages.length === 0 && (
                            <div className="py-8 text-center text-gray-500">
                                ...
                                <p className="mt-2 text-sm">
                                    MCP features will be used automatically and
                                    aggressively
                                </p>
                            </div>
                        )}
                            ...
                            >
                                ...
                                >
                                    ...
                                    {message.resourcesUsed &&
                                        message.resourcesUsed.length > 0 && (
                                            <div className="mt-2 border-gray-300 border-t pt-2 text-xs opacity-75">
                                                <details>
                                                    <summary className="cursor-pointer">
                                                        📄 Used{" "}
