...
function getServer(): McpServer {
    ...
    server.resource(
        ...
        new ResourceTemplate("fs://{locale}/{route}", {
            list: async (): Promise<ListResourcesResult> => { // Auflistung aller Resources
                return {
                    resources: [
                        {
                            name: "All Pages",
        ...
        // Abrufen einer einzelnen Resource
        async (uri, {locale, route}) => {
            return magicHere(locale, route)
        }
        // {
        //     contents: [{
        //         uri: uri.href,
...
async function magicHere(localeString: string | string[], routeString: string | string[]): Promise<ReadResourceResult> { // ReadResourceResult zurückgeben
    /*
